...
class MigrationGenerator {
  ...
  generateMigrationContent(migrationInfo, columns, options = {}) {
    ...
    if (migrationInfo.action === "createJoinTable") {
      console.log("[DEBUG] Generating join table migration with:", {
    ...
    switch (migrationInfo.action) {
      case "create": {
        if (migrationInfo.hasPolymorphic) {
          console.log("[DEBUG] Using polymorphic template for:", {
            tableName: migrationInfo.tableName,
            polymorphicName:
              migrationInfo.polymorphicColumns[0].options.polymorphicName,
          });

          // Filter out the polymorphic columns since they'll be handled by the template
          const nonPolymorphicColumns = columns.filter(
            (col) =>
              !col.name.includes(
                migrationInfo.polymorphicColumns[0].options.polymorphicName
              )
          ...
          console.log(
            "[DEBUG] Non-polymorphic columns:",
            nonPolymorphicColumns
          );
        ...
        return createTableTemplate(migrationInfo.tableName, columns, options);
      }
      case "add":
      ...
      case "renameTable": {
        console.log("[DEBUG] Using rename table template for:", {
          fromTable: migrationInfo.fromTable,
          toTable: migrationInfo.toTable,
        });
      ...
      case "addPolymorphic": {
        console.log("[DEBUG] Using polymorphic template for add:", {
          tableName: migrationInfo.tableName,
          polymorphicName: migrationInfo.polymorphicName,
        });
        ...
        return addPolymorphicTemplate(
          ...
          {
            action: "add",
            columns: columns.filter((col) => !col.options?.polymorphic),
            timestamps: false,
          }
        );
      }
      case "createThroughTable": {
        console.log("[DEBUG] Using through table template for:", {
          tableName: migrationInfo.tableName,
          table1: migrationInfo.table1,
          table2: migrationInfo.table2,
        });
