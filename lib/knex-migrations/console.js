...
function addKnexCommands(repl) {
  ...
  repl.context.createHasOneWithSeed = async (
    name,
    columns = [],
    seedData = [],
    options = {}
    ...
    try {
      ...
      return {
        seedFile: result.seed.path,
        migrationFile: result.migration.path,
      };
    } catch (error) {
      return `Error: ${error.message}`;
    }
  ...
  repl.context.addColumnWithSeed = async (
    name,
    columns = [],
    seedData = [],
    options = {}
    ...
    try {
      const generator = new SeedGenerator();
      const result = await generator.generateAddColumnWithSeed(name, {
        columns,
        data: JSON.stringify(seedData),
        options: {
          timestamps: options.timestamps !== false,
