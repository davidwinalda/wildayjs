...
function displayMultiDatabaseConfig(config) {
  ...
  console.log(cli.info("Configured Databases:"));
  console.log("-------------------");
...
function displayAvailableCommands() {
  console.log("\n");
  console.log(cli.info("Available Commands:"));
  console.log("------------------");
  console.log(
    "1. Initialize multi-database:  " +
      cli.command("wildayjs db:init --type <type>")
  );
  console.log(
    "2. Switch active database:     " + cli.command("wildayjs db:switch <type>")
  );
  console.log(
    "3. Check database status:      " + cli.command("wildayjs db:status")
  );
  console.log(
    "4. Check database connection:  " + cli.command("wildayjs db:check")
  );

  log.info(`Supported database types: ${DATABASE_TYPES.join(", ")}`);
}
