...
function handleResult(value, callback) {
  console.log(
    "[DEBUG] handleResult received value type:",
    value?.constructor?.name
  );

  try {
    // If it's a QueryBuilder, don't execute it
    if (
      value?.constructor?.name === "CustomQueryBuilder" ||
      value?.constructor?.name === "QueryBuilder"
    ) {
      console.log(
        "[DEBUG] handleResult detected QueryBuilder, passing through"
      );
      callback(null, value);
      return;
    }
    ...
    if (Array.isArray(value)) {
      console.log("[DEBUG] Handling array result");
      value.forEach((item) => {
        if (item?.constructor?.prototype instanceof ObjectionModel) {
          Object.setPrototypeOf(item, item.constructor.prototype);
        }
      ...
      console.log("[DEBUG] Handling model instance result");
      Object.setPrototypeOf(value, value.constructor.prototype);
    }

    callback(null, value);
  } catch (error) {
    console.error("[DEBUG] Error in handleResult:", error);
    callback(error);
  }
}
...
function startConsole() {
  ...
  const debugFunction = (enabled = true) => {
    ...
    // Override console.log to show all messages with appropriate formatting
    console.log = function (...args) {
      const message = args[0]?.toString() || "";

      if (enabled) {
        // When debug is enabled, show everything with appropriate colors
        if (message.startsWith("SQL Query:") || message.includes("Bindings:")) {
          // SQL-related messages in yellow
          ...
          // All other messages in default color
          global._originalConsoleLog(...args);
        }
      } else {
        // When debug is disabled, show all logs without filtering
        global._originalConsoleLog(...args);
      }
    };
  ...
  initializeDatabase().then(async () => {
    ...
    const r = repl.start({
      ...
      eval: async function (cmd, context, filename, callback) {
        ...
        const executeCode = async (code) => {
          ...
          try {
            ...
            const shouldExecute =
              originalCmd.startsWith("await") || code.includes(".then(");
            // console.log("[DEBUG] Should execute query:", shouldExecute);

            // Special handling for single variable references
            if (/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(code)) {
              // console.log("[DEBUG] Single variable reference detected");
              const result = eval(`with (r.context) { ${code} }`);

              if (result?.constructor?.name === "CustomQueryBuilder") {
                // console.log("[DEBUG] Variable is QueryBuilder");
                if (shouldExecute) {
                  // console.log("[DEBUG] Executing QueryBuilder");
                  const value = await result;
                  handleResult(value, callback);
                  return;
                }
                // console.log("[DEBUG] Returning QueryBuilder without execution");
            ...
            // Handle assignments directly without async wrapper
            if (code.includes("=") && !code.includes(".then(")) {
              // console.log("[DEBUG] Direct evaluation for assignment");
              const result = eval(`with (r.context) { ${code} }`);

              if (result?.constructor?.name === "CustomQueryBuilder") {
                // console.log("[DEBUG] Assignment returned QueryBuilder");
                callback(null, result);
                return;
              }
            ...
            // For non-assignments or .then() calls, use async wrapper
            const wrapped = `
          (async () => {
            with (r.context) {
              try {
                ...
                if (val?.constructor?.name === "CustomQueryBuilder") {
                  if (${shouldExecute}) {
                    // console.log("[DEBUG] Executing QueryBuilder");
                    return await val;
                  }
                ...
                if (val && typeof val.then === "function") {
                  // console.log("[DEBUG] Promise detected, awaiting");
                  return await val;
                }
        ...
        `;
            ...
            if (result?.constructor?.name === "CustomQueryBuilder") {
              ...
              // console.log("[DEBUG] Handling Promise");
              const value = await result;
              handleResult(value, callback);
            } else {
              handleResult(result, callback);
            ...
            // console.error("[DEBUG] executeCode error:", err);
            callback(err);
          }
        };
        ...
        try {
          // Store if this was an await operation
          const isAwait = cmd.startsWith("await");

          // Handle await keywords
      ...
      writer: function (output) {
        console.log(
          "[DEBUG] Writer received output type:",
        ...
        if (output?.constructor?.name === "CustomQueryBuilder") {
          try {
            const sql = output.toKnexQuery().toString();
            return `QueryBuilder (not executed) {
      SQL: ${sql}
      Type 'await' to execute
    }`;
          } catch (err) {
            console.log("[DEBUG] Error getting SQL:", err);
            return "QueryBuilder { Use 'await' to execute }";
          }
        }
    ...
    // Ctrl+C handler for multi-line input
    r.on("SIGINT", () => {
      if (isMultilineMode) {
        isMultilineMode = false;
        inputBuffer = "";
