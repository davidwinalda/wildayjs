...
class Controller extends Base {
  ...
  // Add render method that includes helpers
  render(res, view, data = {}) {
    const helpers = this.app?.getHelpers() || {};
    const mergedData = {
      ...helpers,
      ...data,
    };

    console.log("\n🎯 Controller Render Debug:");
    console.log("- View:", view);
    console.log("- Helpers:", Object.keys(helpers));
    console.log("- Merged Data:", Object.keys(mergedData));

    return this.app.render(res, view, mergedData);
  }
    ...
    methods.forEach((methodName) => {
      ...
      if (!originalMethod._isWrapped && !this._isCallback(methodName)) {
        const wrappedMethod = async (req, res, render) => {
          try {
            // Set app instance from request handler
            this.app = req.app;
