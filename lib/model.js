...
class Model extends ObjectionModel {
  ...
  static _QueryBuilder = null; // Private property to store the QueryBuilder

  static get QueryBuilder() {
    return this._QueryBuilder || CustomQueryBuilder;
  }
    ...
    qbMethods.forEach((methodName) => {
      ...
      if (typeof this[methodName] === "undefined") {
        this[methodName] = (...args) => {
          console.log(
            `Static method called: ${this.name}.${methodName}(${args.join(
              ", "
            )})`
          );
          return this.query()[methodName](...args);
        };
        console.log(`Registered static method: ${this.name}.${methodName}()`);
      }
    });
  ...
  static async find(id) {
    ...
    const record = await this.query().findById(id);
    if (record) await record.runCallbacks("afterFind");
    return record;
  }

  static async findStrict(id) {
    const record = await this.find(id);
    if (!record) throw new Error(`${this.name} with id ${id} not found`);
    return record;
  }

  static async find_by(conditions) {
    try {
      const record = await this.query().findOne(conditions);
      if (record) await record.runCallbacks("afterFind");
      return record;
    } catch (error) {
      console.error("Error in find_by:", error);
  ...
  static async find_by_strict(conditions) {
    const record = await this.find_by(conditions);
    if (!record) throw new Error(`${this.name} matching conditions not found`);
    return record;
  }

  static async first() {
    const record = await this.query().orderBy("id", "asc").first();
    if (record) await record.runCallbacks("afterFind");
    return record;
  }

  static async last() {
    const record = await this.query().orderBy("id", "desc").first();
    if (record) await record.runCallbacks("afterFind");
    return record;
  }
  ...
  static async take(limit = 1) {
    ...
    const records = await query;
    await Promise.all(
      records.map((record) => record.runCallbacks("afterFind"))
    );
    return records;
  }
  ...
  static async find_each(batchSize = 1000, callback) {
    ...
    do {
      records = await this.query().limit(batchSize).offset(offset);
      for (const record of records) {
        await record.runCallbacks("afterFind");
        await callback(record);
      }
      offset += batchSize;
    } while (records.length === batchSize);
  ...
  static async find_or_create_by(attributes) {
    const record = await this.find_by(attributes);
    if (record) return record;
    return await this.create(attributes);
  }

  static async find_or_initialize_by(attributes) {
    const record = await this.find_by(attributes);
    if (record) return record;
    return new this(attributes);
  }

  static async find_by_sql(sql, bindings = []) {
    const knex = await this.getKnex();
    const results = await knex.raw(sql, bindings);
    const records = results.map((result) => new this(result));
    await Promise.all(
      records.map((record) => record.runCallbacks("afterFind"))
    );
    return records;
  ...
  static async count_by(conditions) {
    return await this.where(conditions).count();
  }
  ...
  static where_json_contained(column, value) {
    return this.query().whereJsonSubsetOf(column, value);
  }
  ...
  static today() {
    ...
    return this.query().whereBetween("created_at", [start, end]);
  }
  ...
  static where_starts_with(column, value) {
    return this.where_like(column, `${value}%`);
  }

  static where_ends_with(column, value) {
    return this.where_like(column, `%${value}`);
  }

  static where_contains(column, value) {
    return this.where_like(column, `%${value}%`);
  }

  // Exists Clauses
  static where_exists(callback) {
    return this.query().whereExists(callback);
  }

  static where_not_exists(callback) {
    return this.query().whereNotExists(callback);
  }

  // Column Comparisons
  static where_column(column1, operator, column2) {
    return this.query().whereColumn(column1, operator, column2);
  }

  static where_column_in(column1, column2) {
    return this.query().whereColumn(column1, "in", column2);
  }
   ...
   */
  static orderByTheme(theme = "recent", column = null, customThemes = {}) {
    return this.query().orderByTheme(theme, column, customThemes);
  }
   ...
   */
  static order_by(column, direction = "asc") {
    return this.query().order_by(column, direction);
  }
   ...
   * @returns {QueryBuilder}
   */
  static reverse_order() {
    return this.query().reverse_order();
  }
   ...
   * @returns {QueryBuilder}
   */
  static reorder(column = "id", direction = "asc") {
   ...
   * @returns {QueryBuilder}
   */
  static order_by_raw(sql, bindings = []) {
    return this.query().order_by_raw(sql, bindings);
  }
   ...
   * @returns {QueryBuilder}
   */
  static nulls_first(column, direction = "asc") {
    return this.query().nulls_first(column, direction);
  }
   ...
   * @returns {QueryBuilder}
   */
  static nulls_last(column, direction = "asc") {
    return this.query().nulls_last(column, direction);
  }
   ...
   * @returns {QueryBuilder}
   */
  static order_by_case(column, cases) {
    return this.query().order_by_case(column, cases);
  }
   ...
   * @returns {QueryBuilder}
   */
  static order_by_field(column, values) {
    return this.query().order_by_field(column, values);
  }
   ...
   * @returns {QueryBuilder}
   */
  static order_by_multiple(orders) {
    return this.query().order_by_multiple(orders);
  }
   ...
   * @returns {QueryBuilder}
   */
  static order_by_relation(relation, column, direction = "asc") {
    return this.query().order_by_relation(relation, column, direction);
  }
   ...
   * @returns {QueryBuilder}
   */
  static order_by_count(relation, direction = "desc") {
    return this.query().order_by_count(relation, direction);
  }
   ...
   * @returns {QueryBuilder}
   */
  static order_by_when(condition, trueOrder, falseOrder) {
    return this.query().order_by_when(condition, trueOrder, falseOrder);
  }
  ...
  static having_raw(sql, bindings = []) {
    return this.query().havingRaw(sql, bindings);
  }
  ...
  static async pluck(column) {
    const results = await this.query().select(column);
  ...
  static async ids() {
    return await this.pluck("id");
  }

  static async pick(column) {
    const result = await this.query().select(column).first();
    return result?.[column];
  }

  static async pick_all(...columns) {
    return await this.query()
      .select(...columns)
      .first();
  }
