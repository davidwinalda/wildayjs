...
class Model extends ObjectionModel {
  ...
  static boot() {
    ...
    // Set the table name if not explicitly set
    if (!this.tableName) {
      this.tableName = pluralize(this.name.toLowerCase());
    }
  ...
  static registerQueryBuilderMethods() {
    // Get all available methods from QueryBuilder
    const qbMethods = this.QueryBuilder.methods;
    console.log(`Found QueryBuilder methods for ${this.name}:`, qbMethods);

    // Register each method as a static method
