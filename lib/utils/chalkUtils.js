...
const errors = {
  ...
  invalidMigrationFormat: () => {
    ...
    console.log("  " + cli.example("RenamePhoneNumberToPhoneInUsers"));
...
${cli.title("WildayJS CLI Usage:")}
  ...
  ${cli.section("Controller Commands:")}
    ${cli.command("generate:controller")} ${cli.param(
      "<name> [actions...]"
    )} ${cli.param("[options]")}
    ${cli.comment("# Generate a new controller")}

    ${cli.comment("# Options:")}
    ${cli.command("--api")}         ${cli.comment(
      "# Generate an API controller"
    )}
    ${cli.command("--skip-views")}  ${cli.comment("# Skip generating views")}
    ${cli.command("--skip-routes")} ${cli.comment("# Skip adding routes")}

    ${cli.comment("# Examples:")}
    ${cli.command("wildayjs generate:controller")} ${cli.example(
      "users index show"
    )}
    ${cli.command("wildayjs generate:controller")} ${cli.example(
      "posts index --api"
  ...
  ${cli.section("Model Commands:")}
    ${cli.command("generate:model")} ${cli.param("<name> [attributes...]")}
    ${cli.comment("# Generate a new model with optional attributes")}

    ${cli.comment("# Basic Examples:")}
    ${cli.command("wildayjs generate:model")} ${cli.example(
      "user"
    )}            ${cli.comment("# Create empty model")}
    ${cli.command("wildayjs generate:model")} ${cli.example(
      "user name:string email:string"
    )}

    ${cli.comment("# With Validations:")}
    ${cli.command("wildayjs generate:model")} ${cli.example(
      "user name:string:null:false email:string:unique"
    )}

    ${cli.comment("# With Relationships:")}
    ${cli.command("wildayjs generate:model")} ${cli.example(
      "post title:string user:references"
    )}
    ${cli.command("wildayjs generate:model")} ${cli.example(
      "comment body:text post:belongs_to user:belongs_to"
    )}
