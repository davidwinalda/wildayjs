...
const generateMigration = (migrationName, columns) => {
  ...
  if (action) {
    ...
    switch (action) {
      case "add":
        try {
          ...
          // Step 4: Copy data from temp to new table
          const newColumnValues = columns.map((column) => {
            ...
            if (defaultModifier) {
              const value = processDefaultValue(defaultModifier.split("=")[1]);
          ...
          downSql += `SELECT ${existingColumns.join(", ")} FROM ${table};\n\n`;
    ...
    const downFileName = `${timestamp}_${toSnakeCase(migrationName)}_down.sql`;
    ...
    const downSql = `DROP TABLE IF EXISTS ${toSnakeCase(migrationName)};`;
