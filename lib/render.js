  ...
  if (viewPath.startsWith("errors/")) {
    try {
      ...
      if (defaultTemplate && fs.existsSync(defaultTemplate)) {
        return ejs.renderFile(defaultTemplate, defaultData, (err, content) => {
          if (err) throw err;
      ...
      if (fs.existsSync(appErrorPath)) {
        return ejs.renderFile(appErrorPath, defaultData, (err, content) => {
          if (err) throw err;
          res.writeHead(status, { "Content-Type": "text/html" });
          res.end(content);
        });
      ...
      return res.end(`
        ...
        <html>
          <head>
            <title>Error - ${defaultData.error.status}</title>
            <style>
              body {
          ...
          <body>
            <div class="error-container">
              <h1 class="error-status">${defaultData.error.status}</h1>
              <p class="error-message">${defaultData.error.message}</p>
              ${
                defaultData.error.detail
                  ? `<pre class="error-detail">${defaultData.error.detail}</pre>`
                  : ""
              }
            </div>
          </body>
